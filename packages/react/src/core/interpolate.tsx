// TODO: See https://github.com/formatjs/formatjs/blob/main/packages/icu-messageformat-parser/README.md
...
export const createReactInterpolate = ({
  ...
  const plugin = createReactComponentsPlugin(locale, components)

  return (props: InterpolateProps): string | ReactNode => {
...
function createReactComponentsPlugin(
  locale: Locale,
  components: InterpolateComponents = {}
) {
  const plugin = (message: string) =>
    new IntlMessageFormat(message, locale, undefined, {
      ...
      blockquote: (children: ReactNode) => <blockquote>{children}</blockquote>,
      ...
      figcaption: (children: ReactNode) => <figcaption>{children}</figcaption>,
