  ...
  return function I18nProvider({
    ...
    useEffect(() => {
      function checkConfigMatch([first, second]: [
        ...
        const notDefined = first[1].filter(
          locale => !second[1].includes(locale)
        )

        if (notDefined.length > 0) {
          warn(
            `The following locales are defined in '${first[0]}' but not in '${
              second[0]
            }': ${notDefined.join(', ')}`
          )
        }
      ...
      const createI18n = ['createI18n', Object.keys(locales)] as [
        string,
        string[]
      ]
      const nextConfig = ['next.config.js', nextLocales || []] as [
        string,
        string[]
      ]
    ...
    const value = useMemo(
      () => ({
        ...
        fallbackLocale: fallbackLocale
          ? flattenLocale<Locale>(fallbackLocale)
          : undefined,
